...
export async function saveToIndexedDB<K extends keyof StoresMap>(
  ...
  if (!db.objectStoreNames.contains(store)) {         // Ensure store exists
...
// GET
export async function getFromIndexedDB<K extends keyof StoresMap>(store: K): Promise<
  | { success: true; data: StoresMap[K] }
