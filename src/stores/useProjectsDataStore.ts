...
export const useProjectsDataStore = create<ProjectsStore>((set) => ({
  ...
  fetchProjects: async () => {
    ...
    try {
      const response = await fetch(                             // Fetch all repo's of `srinivas-batthula`...
        "https://api.github.com/users/srinivas-batthula/repos"
      );
      const repos: any[] = await response.json();

      repos.sort(                                       // Sort repositories by creation date (descending / Newest-1st)...
        (a, b) =>
          new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
      );

      let i = 0;
      const promises = repos.map(async (repo) => {              // Fetch `metadata.json` files from all the repo's...
        try {
      ...
      const data = (await Promise.all(promises)).filter(        // Wait for all fetch requests to finish...
